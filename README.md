...
## 目录结构
1. 目录test存放测试用例，
2. 目录scripts从Package [react-scripts](https://www.npmjs.com/package/react-scripts)复制而来，复用原有的命令
3. 目录config从Package [react-scripts](https://www.npmjs.com/package/react-scripts)复制而来，并进行了相应的修复，主要包括：
  a. 支持SCSS
  b. 支持CSS Module
  c. 支持ES7特性Decorators
4. 目录build为运行以下命令自动生成，为线上运行构建后的包
```
npm run build
```
5. temp目录为MockServer接收文件上传后保持文件的位置
...
```
### Mock数据
1. 启动MockServer,该服务会自动搜索并加载/mock/modules目录下的文件
```
  npm run mock
```
2. MockServer启动端口配置为Package.json中的属性mockServerPort
3. 其中mock目录存放了后端接口Mock数据，使用了[MockJS](http://mockjs.com/examples.html)方便生成数据
4. 参考文件/mock/template.js定义数据
